...
export default async function DashboardPage({ searchParams }: { searchParams?: { view?: string } }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6">
    ...
    {admin && view === 'resumo' ? <section className="space-y-6"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><ResumoCard titulo="Solicitações" valor={requests.length} detalhe="Total" /><ResumoCard titulo="Aguard. recolhimento" valor={pendingCollection} detalhe="Pendentes" /><ResumoCard titulo="Aguard. aprovação" valor={pendingApproval} detalhe="Orçamentos" /><ResumoCard titulo="Clientes ativos" valor={activeClients} detalhe={`${clients.length} cadastrados`} /><ResumoCard titulo="Serviços ativos" valor={activeServices} detalhe={`${services.length} cadastrados`} /></div><section className="card"><div className="flex items-center justify-between"><h2 className="font-semibold">Últimas solicitações</h2><Link href="/dashboard?view=solicitacoes" className="text-sm font-semibold text-mdblue">Ver todas</Link></div><SolicitacoesTabela requests={requests.slice(0, 8)} /></section><section className="grid gap-4 lg:grid-cols-2"><ResumoCard titulo="Convites pendentes" valor={pendingInvites} detalhe="Dentro do prazo" /><ResumoCard titulo="Empresas cadastradas" valor={companies.length} detalhe="Disponíveis no portal" /></section></section> : null}
    ...
    {admin && view === 'solicitacoes' ? <section className="card"><h2 className="font-semibold">Solicitações / acompanhamento</h2><SolicitacoesTabela requests={requests} /></section> : null}
    ...
    {!admin ? <section className="card"><h2 className="font-semibold">Solicitações / acompanhamento</h2><SolicitacoesTabela requests={requests} /></section> : null}
  </main>;
}
...
function SolicitacoesTabela({ requests }: { requests: Array<{ id: string; protocol: string; currentStatus: string; company: { name: string }; requester: { name: string } }> }) {
