...
import { RequestsFilterTable } from '@/components/requests-filter-table';
...
export default async function DashboardPage({ searchParams }: { searchParams?: { view?: string } }) {
  ...
  if (!admin) {
    let requests: Array<{ id: string; protocol: string; currentStatus: string; serial: string; company: { name: string }; requester: { name: string } }> = [];
    try {
    ...
    return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6">
      ...
      <section className="card"><h2 className="font-semibold">Minhas solicitações</h2><RequestsFilterTable requests={requests} companies={[]} /></section>
    </main>;
  }
  ...
  try {
    ...
    if (view === 'resumo') openWarranties = await prisma.warranty.count({ where: { status: { in: [WarrantyStatus.SOLICITACAO_ABERTA, WarrantyStatus.EM_ANALISE] } } });
  } catch (error) {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6">
    ...
    {view === 'resumo' ? <section className="grid gap-4 md:grid-cols-5"><ResumoCard titulo="Solicitações" valor={requests.length} detalhe="Total" /><ResumoCard titulo="Clientes" valor={clients.length} detalhe="Cadastrados" /><ResumoCard titulo="Empresas" valor={companies.length} detalhe="Cadastradas" /><ResumoCard titulo="Serviços" valor={services.length} detalhe="Cadastrados" /><ResumoCard titulo="Garantias abertas" valor={openWarranties} detalhe="Abertas/em análise" /></section> : null}
    {view === 'resumo' ? <section className="card"><h2 className="font-semibold">Solicitações / acompanhamento</h2><p className="mt-1 text-sm text-slate-500">Use o cabeçalho para filtrar digitando ou escolhendo uma opção da própria lista.</p><RequestsFilterTable requests={requests} companies={[]} /></section> : null}
    ...
    {view === 'solicitacoes' ? <section className="card"><h2 className="font-semibold">Solicitações / acompanhamento</h2><p className="mt-1 text-sm text-slate-500">Use o cabeçalho para filtrar digitando ou escolhendo uma opção da própria lista.</p><RequestsFilterTable requests={requests} companies={companies.map((company) => ({ id: company.id, name: company.name }))} /></section> : null}
  </main>;
}
