...
export default async function DashboardPage({ searchParams }: { searchParams?: { view?: string } }) {
  ...
  try {
    ...
    if (['solicitacoes'].includes(view)) requests = await prisma.serviceRequest.findMany({ include: { company: true, requester: true }, orderBy: { createdAt: 'desc' } });
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-6">
    ...
    {view === 'resumo' ? <section className="grid gap-4 md:grid-cols-5"><ResumoCard titulo="Solicitações" valor={requests.length} detalhe="Acesse a aba Solicitações" /><ResumoCard titulo="Clientes" valor={clients.length} detalhe="Cadastrados" /><ResumoCard titulo="Empresas" valor={companies.length} detalhe="Cadastradas" /><ResumoCard titulo="Serviços" valor={services.length} detalhe="Cadastrados" /><ResumoCard titulo="Garantias abertas" valor={openWarranties} detalhe="Abertas/em análise" /></section> : null}
    ...
    {view === 'solicitacoes' ? <section className="card"><h2 className="font-semibold">Solicitações / acompanhamento</h2><RequestsFilterTable requests={requests} companies={companies.map((company) => ({ id: company.id, name: company.name }))} /></section> : null}
  </main>;
}
