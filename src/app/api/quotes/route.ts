...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      const pdfBytes = await withTimeout(generateQuotePdf({ quote, request: current, portalUrl: `${appUrl()}/requests/${requestId}` }), 15000, 'Não foi possível gerar o PDF do orçamento.');
      console.info('[quote-api]', { etapa: 'generate_pdf_done', requestId, quoteId, protocol, pdfBytes: pdfBytes.length });

      const delivery = await withTimeout(sendQuoteEmail({ request: current, quote, pdfBytes }), 15000, 'Tempo limite ao enviar e-mail do orçamento.');
      if (delivery.sent > 0 && delivery.failed === 0 && delivery.fallbackSent === 0) {
        return NextResponse.json({ success: true, status: 'success', message: 'Orçamento criado e enviado ao cliente por e-mail.', quoteId: quote.id });
      }
      if (delivery.sent > 0) {
        return NextResponse.json({ success: false, status: 'warning', etapa: 'send_email', message: 'PDF gerado, mas houve falha ao enviar o e-mail para todos os destinatários.', quoteId: quote.id });
      }
      ...
      await prisma.serviceRequestStatusHistory.create({ data: { serviceRequestId: requestId, fromStatus: ServiceRequestStatus.AGUARDANDO_APROVACAO, toStatus: ServiceRequestStatus.AGUARDANDO_APROVACAO, changedById: user.id, note: etapa === 'generate_pdf' ? 'Orçamento criado, mas houve falha ao gerar o PDF.' : 'PDF gerado, mas houve falha ao enviar o e-mail.' } });
      return NextResponse.json({ success: false, status: 'warning', etapa, message: etapa === 'generate_pdf' ? 'Orçamento criado, mas não foi possível gerar o PDF.' : 'PDF gerado, mas houve falha ao enviar o e-mail.', quoteId: quote.id });
    }
  } catch (error) {
...
async function sendQuoteEmail({ request, quote, pdfBytes }: { request: EmailRequest; quote: EmailQuote; pdfBytes: Uint8Array }) {
  ...
  const body = [`Protocolo: ${request.protocol}`, `Empresa: ${request.company.name}`, `Aparelho: ${request.tipoAparelho} ${request.marca} ${request.modelo}`, `Valor total: ${formatMoney(quote.totalCents)}`, `Acesse o portal para aprovar ou reprovar: ${requestUrl}`, 'O PDF padronizado do orçamento está anexado a este e-mail.'].join('\n');
...
function quotePdfFileName(quote: EmailQuote) {
