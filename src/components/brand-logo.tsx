  ...
  return (
    <div role="img" aria-label={mdBrand.name} className={`inline-flex items-center gap-3 ${className}`}>
      <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-900 shadow-sm">
        <span className="text-[26px] font-black leading-none tracking-tight text-white">M</span>
        <span className="ml-[-2px] text-[26px] font-black leading-none tracking-tight text-white">D</span>
        <span className="absolute -left-3 top-1/2 h-1 w-4 -translate-y-1/2 rounded-full bg-sky-400" />
        <span className="absolute -right-3 top-4 h-1 w-4 rounded-full bg-sky-400" />
        <span className="absolute -right-3 bottom-4 h-1 w-4 rounded-full bg-sky-400" />
